        ...
        if ($user) {
            if (Hash::check($request->password, $user->password)) {
                if (Auth::attempt(['email' => $user->email, 'password' => $request->password])) {
                    // Attempt to log in using Laravel's authentication
                    $http_response = HttpClient::login($user->email, $request->password, env('API_CLIENT_ID'), env('API_CLIENT_SECRET'));
                    $passport = json_decode($http_response);

                    // Debugging: Check the response structure
                    dd($passport); // Use this to inspect the response object
